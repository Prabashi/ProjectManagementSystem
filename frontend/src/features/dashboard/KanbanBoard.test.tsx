...
describe('KanbanBoard', () => {
  it('renders all six status columns', () => {
    render(<KanbanBoard projectId="p1" tickets={[]} isAdmin={false} onEdit={jest.fn()} onDelete={jest.fn()} />);
  ...
  it('places tickets in their respective columns', () => {
    render(<KanbanBoard projectId="p1" tickets={tickets} isAdmin={false} onEdit={jest.fn()} onDelete={jest.fn()} />);
  ...
  it('passes isAdmin down to columns', () => {
    render(<KanbanBoard projectId="p1" tickets={tickets} isAdmin={true} onEdit={jest.fn()} onDelete={jest.fn()} />);
    expect(screen.getAllByRole('button', { name: /delete/i })).toHaveLength(3);
  });

  it('passes drag handlers from useDragAndDrop to each column', () => {
    const { useDragAndDrop } = jest.requireMock('../../hooks/useDragAndDrop');
    render(<KanbanBoard projectId="p1" tickets={[]} isAdmin={false} onEdit={jest.fn()} onDelete={jest.fn()} />);
    expect(useDragAndDrop).toHaveBeenCalledWith('p1');
  });
});
